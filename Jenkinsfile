   ...
   stage('Nexus Upload Stage') {
    ...
    stage('Deploy Stage') {
      ...
      timeout(time: 200, unit: 'SECONDS') {
          ...
          pushToCloudFoundry(
              target: '"+pipeline.getApiEndpoint()+"',
               organization: '"+pipeline.getOrgName()+"',
               cloudSpace: '"+pipeline.getSpace()+"',
                credentialsId: '"+pipeline.getCfCredentials().getCredentialId()).getJenkinsID()+"',
               )
        }
      }
    }
